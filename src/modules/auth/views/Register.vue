...
<template>
  <div class="min-h-screen bg-[var(--color-bg)] text-[var(--color-text-main)]">
    ...
    <main class="mx-auto flex min-h-[calc(100vh-76px)] w-full max-w-xl items-center px-4 py-8">
      <form class="w-full rounded-2xl border border-(--color-border) bg-[var(--color-surface)] p-8 shadow-sm" @submit.prevent="handleRegister">
        ...
        <div class="mt-6 space-y-4">
          <IconField label="Nombre completo" icon="person">
            <input v-model="fullName" class="w-full rounded-lg border border-(--color-border) bg-[var(--color-bg)] py-3 pl-10 pr-4 text-sm" required />
          </IconField>
          <IconField label="Correo" icon="mail">
            <input v-model="email" type="email" class="w-full rounded-lg border border-(--color-border) bg-[var(--color-bg)] py-3 pl-10 pr-4 text-sm" required />
          </IconField>
          <IconField label="Contrasena" icon="lock">
            <input v-model="password" type="password" class="w-full rounded-lg border border-(--color-border) bg-[var(--color-bg)] py-3 pl-10 pr-4 text-sm" required />
          </IconField>
        </div>
